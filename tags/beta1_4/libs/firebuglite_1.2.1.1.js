(function(_scope){
   ...
   pi.util = {
      ...
      Init:[],
      Import:function(_url,_type){
         var ftype = _type||_url.match(/\.(\w+)$/);
         ftype = ftype instanceof Array&&ftype.length>1?ftype[1]:ftype;
         switch(ftype){
            case "js":
               return window.eval.call(window,pi.xhr.get(_url).responseText);
            case "css":
               return new pi.element("link").attribute.set("rel","stylesheet").attribute.set("href",_url).insert( pi.get.byTag("head")[0] );
         };
         throw Error("pi.util.Import("+Array.prototype.join.apply(arguments,",")+"): Unknown filetype, specify absolute file type as give the second argument.");
      },
...
var firebug = {
   ...
   init:function(){
      ...
      with(firebug){
         ...
         el.button.console.clear = new pi.element("A").attribute.addClass("Button").event.addListener("click",pi.util.Curry(d.clean,window,el.left.console.monitor)).update("Clear").insert(el.button.console.container);
   ...
   d: {
      ...
      console:{
         ...
         warn:function(_values){
            with (firebug) {
               ...
               d.console.addLine().attribute.addClass("Warn").update(value);
               d.console.scroll();
         ...
         error:function(_values){
            with (firebug) {
               ...
               d.console.addLine().attribute.addClass("Error").update(value);
               d.console.scroll();
               
            }
         },
         debug:function(_values){
            with (firebug){
      ...
      highlight:function(_value,_inObject,_inArray,_link){
         with(firebug){
            ...
            if(_value==null||pi.util.Array.indexOf(["boolean","function","number","string"], typeof _value)>-1){
               // NULL
               if(_value==null){
               ...
               // BOOLEAN & NUMBER
               if (pi.util.Array.indexOf(["boolean", "number"], typeof _value) > -1) {
      ...
      refreshSize:function(){
         with(firebug){
            el.main.environment.addStyle({ "width":pi.util.GetViewport().width+"px"});
            if(pi.env.ie6)
               el.main.environment.addStyle({ "top":pi.util.GetViewport().height-el.main.environment.getSize().offsetHeight+"px" });
         }
      }
   },
